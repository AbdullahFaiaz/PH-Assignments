  ...
  useEffect(()=>{
    ...
    .then(res => {
      ...
      const handleDonation = () =>{
        axios.post('http://localhost:5000/create-payment',{amount: '15', currency: 'USD'} )
        .then(res=> {
...
{myPosts.length>0? (<div className="w-[90%] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
  {myPosts.map(post=> <div key={post._id} className="flex flex-col items-center justify-center mb-7 mx-auto">
  <img className="w-[320px] h-56 bg-gray-300 rounded-lg shadow-md" src={post.Thumbnail}/>
...
<div className="flex flex-col items-center justify-center">
  <button onClick={handleDonation} className="btn btn-md bg-black text-white">Donate</button>
</div>
